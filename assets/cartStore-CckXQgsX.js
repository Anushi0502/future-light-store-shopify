...
`;function k(e){try{let t=new URL(e);return t.searchParams.set(`channel`,`online_store`),t.toString()}catch{return e}}function A(e=[],t=[]){return e[0]?.message??t[0]?.message??`This item is currently unavailable.`}function j(e,t,n){let r=e?.lines?.edges?.find(e=>e.node?.merchandise?.id===t);return!!(r?.node&&Number(r.node.quantity)>=n&&Number(e?.totalQuantity)>=n)}function M(e){return e.some(e=>e.message.toLowerCase().includes(`cart not found`)||e.message.toLowerCase().includes(`does not exist`))}async function N(e){let t=(await r(T,{input:{lines:[{quantity:e.quantity,merchandiseId:e.variantId}]}}))?.data?.cartCreate,n=t?.userErrors??[],i=t?.warnings??[],a=t?.cart,o=a?.lines?.edges?.find(t=>t.node?.merchandise?.id===e.variantId)?.node?.id;return n.length>0||!a?.checkoutUrl||!o||!j(a,e.variantId,e.quantity)?(console.warn(`Cart creation did not accept the requested item`,{userErrors:n,warnings:i}),{success:!1,message:A(i,n)}):{success:!0,cartId:a.id,checkoutUrl:k(a.checkoutUrl),lineId:o,cart:a}}async function P(e,t){let n=(await r(E,{cartId:e,lines:[{quantity:t.quantity,merchandiseId:t.variantId}]}))?.data?.cartLinesAdd,i=n?.userErrors??[],a=n?.warnings??[];if(M(i))return{success:!1,cartNotFound:!0,message:A(a,i)};if(i.length>0)return console.error(`Add line failed:`,i),{success:!1,message:A(a,i)};let o=n?.cart,s=(o?.lines?.edges??[]).find(e=>e.node.merchandise.id===t.variantId);return j(o,t.variantId,t.quantity)?{success:!0,lineId:s?.node?.id,cart:o}:(console.warn(`Shopify did not accept the requested cart line`,{warnings:a}),{success:!1,message:A(a)})}async function F(e,t,n){let i=(await r(D,{cartId:e,lines:[{id:t,quantity:n}]}))?.data?.cartLinesUpdate,a=i?.userErrors??[],o=i?.warnings??[];if(M(a))return{success:!1,cartNotFound:!0,message:A(o,a)};if(a.length>0)return{success:!1,message:A(o,a)};let s=i?.cart,c=s?.lines?.edges?.find(e=>e.node?.id===t);return!c?.node||Number(c.node.quantity)<n?{success:!1,message:A(o)}:{success:!0,cart:s}}async function I(e,t){let n=(await r(O,{cartId:e,lineIds:[t]}))?.data?.cartLinesRemove,i=n?.userErrors??[],a=n?.warnings??[];return M(i)?{success:!1,cartNotFound:!0}:i.length>0?{success:!1,message:A(a,i)}:{success:!0}}var L=f()(h((e,t)=>({items:[],cartId:null,checkoutUrl:null,isLoading:!1,isSyncing:!1,addItem:async n=>{let{items:r,cartId:i,clearCart:a}=t(),o=r.find(e=>e.variantId===n.variantId),s=null;e({isLoading:!0});try{if(!i){s=S({variantId:n.variantId,quantity:n.quantity,productId:n.product.node.id,productTitle:n.product.node.title,price:n.price});let t=await N({...n,lineId:null});return t.success?(e({cartId:t.cartId,checkoutUrl:t.checkoutUrl,items:[{...n,lineId:t.lineId}]}),s?.resolve(x(t.cart,n.price.currencyCode)),C({productId:n.product.node.id,productTitle:n.product.node.title,variantId:n.variantId,quantity:n.quantity,price:n.price}),{success:!0}):(s?.resolve(null,[{field:[],message:t.message}]),t)}else if(o){let r=o.quantity+n.quantity;if(!o.lineId)return{success:!1,message:`This item is currently unavailable.`};s=S({variantId:n.variantId,quantity:n.quantity,productId:n.product.node.id,productTitle:n.product.node.title,price:n.price});let c=await F(i,o.lineId,r);if(c.success){let i=t().items;return e({items:i.map(e=>e.variantId===n.variantId?{...e,quantity:r}:e)}),s?.resolve(x(c.cart,n.price.currencyCode)),C({productId:n.product.node.id,productTitle:n.product.node.title,variantId:n.variantId,quantity:n.quantity,price:n.price}),{success:!0}}else c.cartNotFound&&a();let l=c.message??`Cart line update failed`;return s?.resolve(null,[{field:[],message:l}]),{success:!1,message:l}}else{s=S({variantId:n.variantId,quantity:n.quantity,productId:n.product.node.id,productTitle:n.product.node.title,price:n.price});let r=await P(i,{...n,lineId:null});if(r.success){let i=t().items;return e({items:[...i,{...n,lineId:r.lineId??null}]}),s?.resolve(x(r.cart,n.price.currencyCode)),C({productId:n.product.node.id,productTitle:n.product.node.title,variantId:n.variantId,quantity:n.quantity,price:n.price}),{success:!0}}else r.cartNotFound&&a();let o=r.message??`Cart line add failed`;return s?.resolve(null,[{field:[],message:o}]),{success:!1,message:o}}}catch(e){return console.error(`Failed to add item:`,e),s?.reject(e),{success:!1,message:`Couldn’t add this item right now.`}}finally{e({isLoading:!1})}},updateQuantity:async(n,r)=>{if(r<=0){await t().removeItem(n);return}let{items:i,cartId:a,clearCart:o}=t(),s=i.find(e=>e.variantId===n);if(!(!s?.lineId||!a)){e({isLoading:!0});try{let i=await F(a,s.lineId,r);if(i.success){let i=t().items;e({items:i.map(e=>e.variantId===n?{...e,quantity:r}:e)})}else i.cartNotFound&&o()}catch(e){console.error(`Failed to update quantity:`,e)}finally{e({isLoading:!1})}}},removeItem:async n=>{let{items:r,cartId:i,clearCart:a}=t(),o=r.find(e=>e.variantId===n);if(!(!o?.lineId||!i)){e({isLoading:!0});try{let r=await I(i,o.lineId);if(r.success){let r=t().items.filter(e=>e.variantId!==n);r.length===0?a():e({items:r})}else r.cartNotFound&&a()}catch(e){console.error(`Failed to remove item:`,e)}finally{e({isLoading:!1})}}},clearCart:()=>e({items:[],cartId:null,checkoutUrl:null}),getCheckoutUrl:()=>t().checkoutUrl,syncCart:async()=>{let{cartId:n,isSyncing:i,clearCart:a}=t();if(!(!n||i)){e({isSyncing:!0});try{let e=await r(w,{id:n});if(!e?.data||!Object.prototype.hasOwnProperty.call(e.data,`cart`))return;let t=e.data.cart;if(t===null){a();return}t?.id===n&&t.totalQuantity===0&&a()}catch(e){console.error(`Failed to sync cart:`,e)}finally{e({isSyncing:!1})}}}}),{name:`vs-cart`,storage:p(()=>localStorage),partialize:e=>({items:e.items,cartId:e.cartId,checkoutUrl:e.checkoutUrl})}));export{f as a,h as i,b as n,a as o,p as r,i as s,L as t};